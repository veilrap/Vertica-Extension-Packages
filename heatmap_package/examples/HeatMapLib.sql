...
SELECT heat_map_image(x,y,c USING PARAMETERS inf=:cs_png,outf='/tmp/de_dust2_contour_overlay.png',overlay=True,contour=4,gaussian=True,xflip=True,yflip=True) OVER() AS (Success) FROM (SELECT heat_map(x,y USING PARAMETERS xbins=50,ybins=50,xmin=-2240,ymin=-1080,xmax=1800,ymax=3320,normalize=3,bounding_box=True,gaussian=True) OVER() AS (x,y,x_t,y_t,c) FROM de_dust2) AS heat_map;
--SELECT heat_map(x,y USING PARAMETERS xbins=50,ybins=50,normalize=3,bounding_box=True,gaussian=False) OVER() AS (x,y,x_t,y_t,c) FROM de_dust2 AS heatmap LIMIT 50;
--SELECT heat_map_image(x,y,c USING PARAMETERS outf='/tmp/testimage.png',contour=0,gaussian=False,output_rows=True) OVER () AS (x,y,r,g,b,a) FROM (SELECT heat_map(x,y USING PARAMETERS xbins=50,ybins=50,normalize=1,bounding_box=True,gaussian=True) OVER() AS (x,y,x_t,y_t,c) FROM de_dust2) AS heat_map;
-- Cleanup
DROP TABLE T;
DROP TABLE de_dust2;
